...
class ValueNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(2, 16) # input state
        self.fc2 = nn.Linear(16, 16) # hidden layer
        self.value = nn.Linear(16, 1) # output value

    def forward(self, state):
        x = T.relu(self.fc1(state)) # 2D -> 16D
        x = T.relu(self.fc2(x)) # 16D -> 16D
        value = self.value(x) # 16D -> 1D
        return value




def run_episode(env, policy, value_net, render=False):
    state, _ = env.reset()
    done = False
    total_reward = 0
    values = []
...
def main():
    ...
    if args.mode == 'visualize':
        ...
        run_episode(env_vis, policy, value_net, render=True)
        env_vis.close()
        return
    ...
    policy = PolicyNetwork()
    value_net = ValueNetwork()
    policy_optimizer = T.optim.Adam(policy.parameters(), lr=3e-4)
    value_optimizer = T.optim.Adam(value_net.parameters(), lr=1e-3)
    num_episodes = 1000
    rewards_history = []
    for episode in range(num_episodes):
        ...
        values = T.cat(values).squeeze()  # shape: [episode_length]
        advantages = returns - values.detach()  # Detach so gradients don't flow into critic
        ...
        # Value (critic) loss
        value_loss = T.nn.functional.mse_loss(values, returns)
